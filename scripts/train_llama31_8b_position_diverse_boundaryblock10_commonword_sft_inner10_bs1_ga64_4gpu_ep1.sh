...
CONFIG_FILE="${CONFIG_FILE:-configs/training/llama3.1_8b_position_diverse_boundaryblock10_commonword_sft_inner10_bs1_ga64_4gpu_ep1.yaml}"
...
OUTPUT_DIR="${OUTPUT_DIR:-/tmp/secalign_runs/llama31_8b_position_diverse_boundaryblock10_commonword_sft_inner10_bs1_ga64_4gpu_epoch1}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/position_diverse_boundaryblock10_commonword_8b_inner10_train_outputs}"
...
ATTACK_INNER_STEPS="${ATTACK_INNER_STEPS:-10}"
...
echo "Launching Llama-3.1-8B boundary-block position-diverse SFT attacker, inner10"
...
python -m torchtune._cli.tune run \
  ...
  "attack_token_mode=random_structural_boundary_block" \
...
echo "Finished Llama-3.1-8B boundary-block position-diverse SFT-attacker inner10 training."
echo "Log: $LOG_FILE"

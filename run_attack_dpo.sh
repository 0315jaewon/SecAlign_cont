...
NUM_ATTACK_TOKENS="${NUM_ATTACK_TOKENS:-10}"
ATTACK_TOKENS_PER_SAMPLE="${ATTACK_TOKENS_PER_SAMPLE:-null}"
...
ATTACK_TOKEN_MODE="${ATTACK_TOKEN_MODE:-suffix}"
ATTACKER_REFERENCE_FREE="${ATTACKER_REFERENCE_FREE:-False}"
ATTACKER_OBJECTIVE="${ATTACKER_OBJECTIVE:-}"
LOG_ATTACK_TOKEN_DIAGNOSTICS="${LOG_ATTACK_TOKEN_DIAGNOSTICS:-False}"
...
if [[ -z "$ATTACKER_OBJECTIVE" ]]; then
  if [[ "$ATTACKER_REFERENCE_FREE" == "True" ]]; then
    ATTACKER_OBJECTIVE="dpo_reference_free"
  else
    ATTACKER_OBJECTIVE="dpo"
  fi
fi
...
CMD=(
  ...
  "attacker_objective=$ATTACKER_OBJECTIVE"
  "log_attack_token_diagnostics=$LOG_ATTACK_TOKEN_DIAGNOSTICS"
...
echo "NUM_ATTACK_TOKENS=$NUM_ATTACK_TOKENS"
echo "ATTACK_TOKENS_PER_SAMPLE=$ATTACK_TOKENS_PER_SAMPLE"
...
echo "ATTACK_TOKEN_MODE=$ATTACK_TOKEN_MODE"
echo "ATTACKER_REFERENCE_FREE=$ATTACKER_REFERENCE_FREE"
echo "ATTACKER_OBJECTIVE=$ATTACKER_OBJECTIVE"
echo "LOG_ATTACK_TOKEN_DIAGNOSTICS=$LOG_ATTACK_TOKEN_DIAGNOSTICS"
echo
printf 'Command:'
printf ' %q' "${CMD[@]}"
printf '\n\n'

...
mkdir -p csa10_oldstyle_curriculum20to50_train_outputs
...
OUTPUT_DIR="${OUTPUT_DIR:-/tmp/secalign_runs/csa10_oldstyle_sft_curriculum20to50_bs128eff_epoch1}" \
EPOCHS="${EPOCHS:-1}" \
...
bash run_attack_dpo.sh \
  "attack_inner_steps_switch_step=${ATTACK_INNER_STEPS_SWITCH_STEP:-75}" \
  "attack_inner_steps_after_switch=${ATTACK_INNER_STEPS_AFTER_SWITCH:-50}"
